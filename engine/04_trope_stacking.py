"""
Script 04: Trope Stacking (The Combo Breaker)
==============================================
Inputs:
  - data/processed/epoch_1_baseline.parquet
  - data/processed/epoch_2_backtest.parquet OR epoch_3_forecast.parquet
Output:
  - A definitive ranking of Rising Trope Combinations (.csv)

The Math:
  Same trend lift and volume filter as Script 03, applied to co-occurring
  pairs (A + B) on the same book (both trope columns == 1).
"""
...
def get_stack_shares(parquet_path: str, epoch_name: str) -> pd.DataFrame:
    """Calculates the percentage of books that contain BOTH tropes in a pair."""
    ...
    pairs = list(itertools.combinations(trope_cols, 2))

    epoch_data = []
    for col_a, col_b in pairs:
        both = (df[col_a].astype(bool) & df[col_b].astype(bool))
        count = int(both.sum())
        share = count / total_books if total_books > 0 else 0

        name_a = col_a.replace("trope_", "")
        name_b = col_b.replace("trope_", "")
        stack_name = (
            f"{name_a} + {name_b}" if name_a < name_b else f"{name_b} + {name_a}"
        )
    ...
    print(
        f"  -> Analyzed {len(pairs):,} unique combinations across {total_books:,} books."
    )
    return pd.DataFrame(epoch_data)


def calculate_stack_lift(
    baseline_df: pd.DataFrame,
    modern_df: pd.DataFrame,
    modern_name: str,
    min_books: int = 5,
) -> pd.DataFrame:
    """Merges the epochs and applies the Threnne Math to the stacks."""
    print("\nCalculating Stack Lift...")

    df = pd.merge(baseline_df, modern_df, on="stack", how="inner")
    ...
    df["baseline_share"] = (df["baseline_share"] * 100).round(4)
    df[f"{modern_name}_share"] = (df[f"{modern_name}_share"] * 100).round(4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--modern_name", default="modern")
    parser.add_argument("--output", required=True, help="Where to save the final report CSV")
    parser.add_argument(
        "--min_books",
        type=int,
        default=3,
        help="Minimum modern books featuring the stack",
    )
    args = parser.parse_args()

    baseline_df = get_stack_shares(args.baseline, "baseline")
    modern_df = get_stack_shares(args.modern, args.modern_name)

    final_report = calculate_stack_lift(
        baseline_df,
        modern_df,
    ...
    print(f"\n🔥 TOP 15 RISING TROPE STACKS (vs Baseline) 🔥")
    print(f"{'Rank':<5} | {'Stack':<45} | {'Lift':<6} | {'Books'}")
    print("-" * 75)

    for idx, row in final_report.head(15).reset_index(drop=True).iterrows():
        lift_str = f"{row['trend_lift']}x"
        print(
            f"{(idx + 1):<5} | {row['stack']:<45} | {lift_str:<6} | "
